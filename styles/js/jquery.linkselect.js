...
;(function($){
	...
	$.LinkSelect = function (el, options){
		...
		var setValuePreviousValue;
		// this updates the DOM element and announces the update
		function setValue(value, text){
			...
			$input.triggerHandler("update", [value, text, self]);
			$select.triggerHandler("update", [value, text, self]);

			// update the last value			
			setValuePreviousValue = value;
		}
		...
		$ul
			...
			.delegate("li", "click.linkselect", function (e, nofocus, doCallback){
				...
				var $previous = findItem(setValuePreviousValue).removeClass(options.classes.selected)
					...
					, previous = {value: (setValuePreviousValue || ""), text: $previous.find("." + options.classes.value).html()};
				// hide the container
				hideOptions(nofocus);
				...
				if( (doCallback !== false) && (($.isFunction(options.change) && (options.change.apply(self, [this, value, text, doCallback]) === false)) || ($.isFunction($select[0].onchange) && ($select[0].onchange.apply(self, [this, value, text, doCallback]) === false))) ){
					...
					setValue(previous.value, previous.text);
					return;
				}
		...
		function findItem(value){
			...
			return $ul.find("li[data-value='" + (value || "") + "']");
		}

		// set the currently selected item
		function setSelectedItem(value, doCallback){
			// look for the currently selected item
			var $selected = findItem(value);
		...
		setValuePreviousValue = this.val();
